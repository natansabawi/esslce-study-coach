...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    """Derives cognitive, temporal, and syllabus-coverage metrics."""
    data = df.copy()

    # Base accuracy and error metrics
    ...
    # Study pacing and engagement velocity
    ...
    # Ebbinghaus-style exponential memory retention proxy
    # R = e^(-t / S), where S is reinforced by practice duration and accuracy
    ...
    # Aggregated student-level context (group-level features)
    ...
    # Relative performance against personal average
    data["acc_vs_student_avg"] = data["accuracy"] - data["student_mean_acc"]

    # One-hot encode categorical subject markers
...
def build_stacked_ensemble():
    """Constructs a cross-validated multi-model stacking regressor."""
    estimators = [
        (
            "lgb",
            lgb.LGBMRegressor(
...
def train(csv_path: str = "student_progress.csv", output_model: str = "esslce_predictor.joblib"):
    ...
    # Filter rows with ground-truth final exam results
    df = df.dropna(subset=["final_exam_score"])
    if len(df) < 20:
        print(f"Insufficient labeled training samples ({len(df)} found). Collect more exam results.")
        return
    ...
    # GroupKFold ensures whole students stay out-of-fold
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups), 1):
        ...
        print(f"Fold {fold} - RMSE: {fold_rmse:.2f} | R²: {fold_r2:.3f}")
